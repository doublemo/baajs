...
export class LocalStream extends MediaStream {
    ...
    publish(transport: Publisher | Subscriber) {
      this.pc = transport.pc;
      this.api = transport.datachannel;
      console.log("bind publish");
...
export function makeRemote(stream: MediaStream, transport: Publisher | Subscriber): RemoteStream {
    ...
    const select = () => {
      ...
      if (transport.datachannel) {
        if (transport.datachannel.readyState !== 'open') {
          // queue call if we aren't open yet
          transport.datachannel.onopen = () => transport.datachannel?.send(JSON.stringify(call));
        } else {
          transport.datachannel.send(JSON.stringify(call));
        }
      }
    };
